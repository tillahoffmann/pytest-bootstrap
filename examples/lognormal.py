...
import numpy as np
import pytest
from pytest_bootstrap import bootstrap_test, BootstrapTestError
...
fig, ax = plt.subplots()
ax.hist(result['statistics'], color='silver', bins=20, label='bootstrapped mean', density=True)
ax.axvline(reference, color='C0', label='lognormal_expectation')
ax.axvline(reference_wrong, color='C1', label='lognormal_expectation_wrong')
